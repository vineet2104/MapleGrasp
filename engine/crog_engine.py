...
def train_with_grasp(train_loader, model, optimizer, scheduler, scaler, epoch, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        if("CLIPSEG" in args.exp_name or "LGRCONVNET" in args.exp_name): # For clipseg the input text needs to be a list of strings
        ...
        if(args.use_max_pool):
            infer_mask = data['infer_mask'].cuda(non_blocking=True).unsqueeze(1)
        else:
            infer_mask = None
...
def validate_with_grasp(val_loader, model, epoch, args):
    ...
    for cnt,data in enumerate(pbar):
        ...
        for idx in range(ins_mask_preds.shape[0]):
            ...
            #print("grasp_qua_mask_pred shape = ",grasp_qua_mask_pred.shape)
            ...
            # print("ins mask target shape", ins_mask_target.shape) # 640,640

            # print("ins_mask_pred shape = ",ins_mask_pred.shape) 640,640
            ...
            #print("grasp qua mask pred shape = ",grasp_qua_mask_pred.shape) # (480,640)
            # print("grasp sin mask pred shape = ",grasp_sin_mask_pred.shape) # (480,640)
            # print("grasp cos mask pred shape = ",grasp_cos_mask_pred.shape) # (480,640)
            # print("grasp wid mask pred shape = ",grasp_wid_mask_pred.shape) # (480,640)
            
            # Calculate grasp configurations
            for i in range(len(num_grasps)):
                num_g = num_grasps[i]
                grasp_preds, _ = detect_grasps(grasp_qua_mask_pred, grasp_sin_mask_pred, grasp_cos_mask_pred, grasp_wid_mask_pred, num_g,verbose=0)

                j_index = calculate_jacquard_index(grasp_preds, grasp_target,shape=grasp_qua_mask_pred.shape,verbose=0)
        ...
        for idx in range(ins_mask_preds.shape[0]):
            ...
            for i in range(len(num_grasps)):
                ...
                if args.visualize and num_g==1:
                    ...
                    counter+=1
                    #visualization(img,ins_mask_target,(grasp_qua_mask_target,grasp_ang_mask_target,grasp_wid_mask_target),grasp_targets_,sent,save_path=os.path.join("./results", args.exp_name, f"results_{cnt}_{num_g}_iou={iou}_jindex={j_index}_grasps_targets.png"))
