...
def smooth_l1_loss_with_weight(pred, target, weight=None, reduction='mean'):
    """
    A custom Smooth L1 (Huber) that supports pixelwise weighting.
    """
    diff = torch.abs(pred - target)
    l = torch.where(diff < 1, 0.5 * diff**2, diff - 0.5)
...
class CROG(nn.Module):
    def __init__(self, cfg):
        ...
        self.use_max_pool = cfg.use_max_pool
        # Vision & Text Encoder
        ...
        if self.use_grasp_masks:
            ...
            self.proj = MultiTaskProjector(cfg.word_dim, cfg.vis_dim // 2, 3,cfg.use_max_pool)
        else:
            print("Disable grasp masks")
            self.proj = Projector(cfg.word_dim, cfg.vis_dim // 2, 3)

    def forward(self, img, word, mask=None, grasp_qua_mask=None, grasp_sin_mask=None, grasp_cos_mask=None, grasp_wid_mask=None,infer_mask=None):
        '''
            img: b, 3, h, w
            word: b, words
            word_mask: b, words
        ...
        if self.use_grasp_masks:
            ...
            # if use_max_pool is False, only use the mask predictions (pred) to compute loss. 
            # If use max pool is True, use the mask predictions as mask pooling for grasp prediction and compute loss using all 4 branches
            
            # b, 1, 104, 104
            pred, grasp_qua_pred, grasp_sin_pred, grasp_cos_pred, grasp_wid_pred = self.proj(fq, state,infer_mask)
            ...
            #pred, grasp_qua_pred, grasp_sin_pred, grasp_cos_pred, grasp_wid_pred -> (bs,1,104,104)
            ...
            if self.training:
                ...
                # Modifications for smaller libero rgs dataset
                # alpha = 10
                # grasp_weight = 1.0 + alpha * (grasp_qua_mask > 0).float()
                # grasp_qua_loss = smooth_l1_loss_with_weight(grasp_qua_pred,grasp_qua_mask,weight=grasp_weight)
                # grasp_sin_loss = smooth_l1_loss_with_weight(grasp_sin_pred,grasp_sin_mask,weight=grasp_weight)
                # grasp_cos_loss = smooth_l1_loss_with_weight(grasp_cos_pred,grasp_cos_mask,weight=grasp_weight)
                # grasp_wid_loss = smooth_l1_loss_with_weight(grasp_wid_pred,grasp_wid_mask,weight=grasp_weight)
