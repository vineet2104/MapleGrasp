...
import numpy as np
from .layers import FPN, Projector, TransformerDecoder, MultiTaskProjectorPP
...
class MAPLEGRASP(nn.Module):
    def __init__(self, cfg):
        ...
        self.use_gt_obj_masks = cfg.use_gt_obj_masks
        # Vision & Text Encoder
        ...
        self.proj = MultiTaskProjectorPP(cfg.word_dim, cfg.vis_dim // 2, 3,self.stage1,self.stage2,self.use_gt_obj_masks)
    ...
    def forward(self, img, word, mask=None, grasp_qua_mask=None, grasp_sin_mask=None, grasp_cos_mask=None, grasp_wid_mask=None):
        ...
        pred, grasp_qua_pred, grasp_sin_pred, grasp_cos_pred, grasp_wid_pred = self.proj(fq, state,mask)
        
        

        if(self.stage1 and not self.stage2): # Compute Loss for m_ins, set other loss to 0
        ...
        elif(self.stage2 and not self.stage1): # Compute loss for all parts
            ...
            if self.training:
                ...
                weight = mask * 0.5 + 1
                alpha = 10
                grasp_weight = 1.0 + alpha * (grasp_qua_pred > 0).float()
