...
#     #print("grasp targets = ",grasp_targets)
#     #print("grasp_preds = ",grasp_preds)
...
def detect_grasps(grasp_quality_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask, num_grasps=5,verbose=0):
    """Detect candidate grasps from the given grasp maps."""
    if(verbose==1):
        print("[DEBUG] detect_grasps() called")
        print("[DEBUG] grasp_quality_mask shape:", grasp_quality_mask.shape, 
            " min:", np.min(grasp_quality_mask), " max:", np.max(grasp_quality_mask))
        print("[DEBUG] grasp_sin_mask shape:", grasp_sin_mask.shape,
            " min:", np.min(grasp_sin_mask), " max:", np.max(grasp_sin_mask))
        print("[DEBUG] grasp_cos_mask shape:", grasp_cos_mask.shape,
            " min:", np.min(grasp_cos_mask), " max:", np.max(grasp_cos_mask))
        print("[DEBUG] grasp_wid_mask shape:", grasp_wid_mask.shape,
            " min:", np.min(grasp_wid_mask), " max:", np.max(grasp_wid_mask))
        print("[DEBUG] num_grasps =", num_grasps)
    ...
    if(verbose==1):
        print("[DEBUG] local_max peaks found:", len(local_max), local_max)

    # Convert sin/cos into angle
    grasp_angle_mask = (np.arctan2(grasp_sin_mask, grasp_cos_mask) / 2.0)

    for p_array in local_max:
        grasp_point = tuple(p_array)  # (row, col)
        grasp_angle = grasp_angle_mask[grasp_point] / np.pi * 180
        grasp_width = grasp_wid_mask[grasp_point]
        
        scaled_width = grasp_width * max_width
        # Build rectangle [x, y, width, height, angle]
        rect = [
            float(grasp_point[1]),  # x_center
            float(grasp_point[0]),  # y_center
            scaled_width,
            20,                    # fixed height
            grasp_angle
        ]
        ...
        if(verbose==1):
            print(f"[DEBUG] local max @ {grasp_point}, angle={grasp_angle:.2f} deg, "
                f"width(normalized)={grasp_width:.3f}, scaled={scaled_width:.3f}, rect={rect}")

    if(verbose==1):
        print("[DEBUG] detect_grasps() returning", len(grasps), "grasp candidates.\n")
    return grasps, grasp_angle_mask


def calculate_iou(rect_p, rect_gt, shape=(480, 640), angle_threshold=30,verbose=0):
    """
    Calculate the IOU between two grasp rectangles in (cx,cy,w,h,theta) format,
    with optional angle threshold check.
    """
    if(verbose==1):
        print("[DEBUG] calculate_iou() called")
        print("[DEBUG] rect_p:", rect_p, " rect_gt:", rect_gt, " shape:", shape, 
            " angle_threshold:", angle_threshold)

    # Check angle difference
    angle_diff_1 = abs(rect_p[4] - rect_gt[4])
    angle_diff_2 = abs(rect_p[4] + rect_gt[4])
    if(verbose==1):
        print(f"[DEBUG] angle_diff_1 = {angle_diff_1:.2f}, angle_diff_2 = {angle_diff_2:.2f}")
    if angle_diff_1 > angle_threshold and angle_diff_2 > angle_threshold:
        if(verbose==1):
            print("[DEBUG] Angle difference too large, returning 0.\n")
        return 0
    ...
    # Ensure indexing remains in-bounds
    mask_rr = rr1 < shape[1]
    rr1 = rr1[mask_rr]
    cc1 = cc1[mask_rr]
    mask_cc = cc1 < shape[0]
    ...
    if(verbose==1):
        print("[DEBUG] GT box corners:", gt_box, 
            " rr1 range:", (rr1.min() if rr1.size > 0 else None, rr1.max() if rr1.size > 0 else None),
            " cc1 range:", (cc1.min() if cc1.size > 0 else None, cc1.max() if cc1.size > 0 else None))

    # Unpack predicted rectangle
    center_x, center_y, w_rect, h_rect, theta = rect_p
    p_r_rect = ((center_x, center_y), (w_rect, h_rect), -theta)
    p_box = cv2.boxPoints(p_r_rect)
    p_box = np.int0(p_box)
    ...
    if(verbose==1):
        print("[DEBUG] Predicted box corners:", p_box,
            " rr2 range:", (rr2.min() if rr2.size > 0 else None, rr2.max() if rr2.size > 0 else None),
            " cc2 range:", (cc2.min() if cc2.size > 0 else None, cc2.max() if cc2.size > 0 else None))
    ...
    if(verbose==1):
        print("[DEBUG] union:", union, " intersection:", intersection)
    if union <= 0:
        if(verbose==1):
            print("[DEBUG] union <= 0, returning 0.\n")
        return 0
    else:
...
def calculate_max_iou(rects_p, rects_gt,shape=(480,640),verbose=0):
    """
    For each predicted rectangle, compute IOU with each ground-truth rectangle,
    and return the maximum IOU found.
    """
    if(verbose==1):
        print("[DEBUG] calculate_max_iou() called")
        print("[DEBUG] # predicted rects:", len(rects_p), " # ground-truth rects:", len(rects_gt))
    max_iou = 0.0
    for i, rect_gt in enumerate(rects_gt):
        for j, rect_p in enumerate(rects_p):
            iou = calculate_iou(rect_p, rect_gt,shape=shape,verbose=verbose)
            # Debugging each pair
            if(verbose==1):
                print(f"[DEBUG] iou for p#{j} vs gt#{i} =", iou)
            if iou > max_iou:
                max_iou = iou
                if(verbose==1):
                    print("[DEBUG] New max_iou =", max_iou)
    if(verbose==1):
        print("[DEBUG] calculate_max_iou() returning max_iou =", max_iou, "\n")
    return max_iou


def calculate_jacquard_index(grasp_preds, grasp_targets, iou_threshold=0.25,shape=(480,640),verbose=0):
    """
    The Jacquard index is 1 if the maximum IOU among (preds vs gt) is > iou_threshold, else 0.
    """
    if(verbose==1):
        print("[DEBUG] calculate_jacquard_index() called")
        print("[DEBUG] # predicted grasps:", len(grasp_preds), 
            " # gt grasps:", len(grasp_targets), 
            " iou_threshold:", iou_threshold)

    grasp_preds = np.asarray(grasp_preds, dtype=np.float32)
    grasp_targets = np.asarray(grasp_targets, dtype=np.float32)

    # Force GT rects to have height=20, clip width to [0,100]
    if(verbose==1):
        print("[DEBUG] Original GT grasp_targets:")
        for idx, g in enumerate(grasp_targets):
            print(f"  gt[{idx}]: {g}")
    ...
    if(verbose==1):
        print("[DEBUG] Modified GT grasp_targets (height=20, width clipped [0,100]):")
        for idx, g in enumerate(grasp_targets):
            print(f"  gt[{idx}]: {g}")

    iou = calculate_max_iou(grasp_preds, grasp_targets,shape=shape,verbose=verbose)

    j_index = 1 if iou > iou_threshold else 0
    if(verbose==1):
        print(f"[DEBUG] Final iou = {iou:.4f}, returning j_index={j_index}\n")
    return j_index

def center_distance_success(
        grasp_preds,
        grasp_targets,
    ...
    """
    Binary success metric for 4-DoF grasps based on centre proximity.
    
    Args
    ----
    grasp_preds    : array-like, shape (N_p, 5) or (5,)   (cx, cy, w, h, theta)
    grasp_targets  : array-like, shape (N_gt, 5) or (5,)
    dist_threshold : float, success if centre distance < threshold (pixels)
    return_distance: if True, also return the minimum centre distance
    verbose        : 1 for debug prints
    
    Returns
    -------
    success  (int) 1 if any pred–gt pair is within `dist_threshold`, else 0
    (optionally)   the minimum Euclidean distance found
    """
    # Convert to (N, 5) float32 arrays
    preds = np.atleast_2d(np.asarray(grasp_preds,  dtype=np.float32))
    gts   = np.atleast_2d(np.asarray(grasp_targets, dtype=np.float32))

    if verbose:
        print(f"[DEBUG] #preds={len(preds)}, #gts={len(gts)}, dist_threshold={dist_threshold}")

    # Extract centre coordinates
    pred_centres = preds[:, :2]   # (N_p, 2)
    gt_centres   = gts[:,  :2]    # (N_gt, 2)

    # Compute all pairwise squared distances -- avoids redundant sqrt
    diff = pred_centres[:, None, :] - gt_centres[None, :, :]   # (N_p, N_gt, 2)
    sq_dists = np.sum(diff ** 2, axis=-1)                      # (N_p, N_gt)

    min_sq_dist = float(np.min(sq_dists))          # best match over all pairs
    success = int(min_sq_dist < dist_threshold ** 2)

    if verbose:
        best_pair = np.unravel_index(np.argmin(sq_dists), sq_dists.shape)
        print(f"[DEBUG] best_pred#{best_pair[0]} vs gt#{best_pair[1]} -> "
              f"dist={np.sqrt(min_sq_dist):.2f} px → success={success}")

    if return_distance:
        return success, np.sqrt(min_sq_dist)
    return success